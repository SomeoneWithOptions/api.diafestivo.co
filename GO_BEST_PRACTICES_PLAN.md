...
- Current module: `go 1.24.3` in `go.mod`.
- `go test ./...`: passes in `golang:1.26.4-alpine`, no tests exist.
- `go vet ./...`: passes in `golang:1.26.4-alpine`.
- `staticcheck ./...`: passes with latest `staticcheck` under Go 1.26.4.
- `go fix -diff ./...`: no suggested modernizer diff under Go 1.26.4.
- `gofmt -l .`: reports `giphy/structs.go`, `ipinfo.go`.
- Local host has no `go` binary; Docker used for checks.
...
- [ ] Update `go.mod` from `go 1.24.3` to `go 1.26.4`.
...
- [ ] Pin Docker builder image to `golang:1.26.4-alpine`.
...
- [ ] Add route contract tests using `httptest`.
...
- [ ] Add deterministic clock seam for tests only.
...
- [ ] Add CI test job before deploy job.
...
- [ ] Replace package-level default mux with explicit `http.NewServeMux()`.
...
- [ ] Replace `http.ListenAndServe` with configured `http.Server`.
...
- [ ] Build server address with `net.JoinHostPort("", port)`.
...
- [ ] Add `writeJSON(w, status, value)` helper.
...
- [ ] Remove `panic` from `giphy.FetchGifURL` path.
...
- [ ] Make JSON decode errors explicit in Giphy/IPInfo.
...
- [ ] Keep exported function signatures for now.
...
- [ ] Protect cached holiday slices from mutation.
...
- [ ] Add unit tests for Colombian holiday dates.
...
- [ ] Add unit tests for `DaysUntil`, `IsToday`, `FindNext`, `GetRemaining` with frozen clock.
  - Preserve current semantics, including whether today's holiday appears in `/left`.
  - Non-breaking guard: detects behavior drift.

- [ ] Replace constructor internals with composite literals.
...
- [ ] Rename constructor params to idiomatic lowerCamel.
  - `is_today` -> `isToday`, `days_until` -> `daysUntil`.
  - Non-breaking guard: param names not part of Go API compatibility.

- [ ] Use typed errors where handler behavior branches on error kind.
...
- [ ] Do naming audit before renames.
...
- [ ] Rename internal HTTP handlers to idiomatic unexported names.
...
- [ ] Rename response/data types to clearer names.
...
- [ ] Rename local vars for clarity and idiomatic Go.
...
- [ ] Rename packages only when value beats churn.
...
- [ ] Consider moving external API clients into focused internal packages.
...
- [ ] Split large/ambiguous files into role-based files.
...
- [ ] Rename constructor funcs or remove unnecessary constructors.
...
- [ ] Normalize acronym/initialism style.
...
- [ ] Keep route path names unchanged.
...
### P2 — API docs / contract docs

- [ ] Fix README examples to match actual JSON.
  - `/make` sample currently appears to swap `name` and `date`.
  - Non-breaking guard: docs only.

- [ ] Document all current public routes.
  - Add `/template`, `/left`, invalid route behavior.
  - Non-breaking guard: docs only.

- [ ] Add OpenAPI or `docs/api.md` contract file.
  - Include status codes, response schemas, examples.
  - Non-breaking guard: docs only; used by tests.

### P2 — Build / Docker / release hygiene

- [ ] Pin Docker build image to target Go version.
  - Use `golang:1.26.4-alpine`.
  - Non-breaking guard: reproducible builds.

- [ ] Decide whether to add `toolchain go1.26.4`.
...
- [ ] Add `.dockerignore`.
  - Exclude `.git`, local temp files, plan docs if desired.
  - Non-breaking guard: smaller build context only.

- [ ] Build binary with reproducible/smaller flags.
...
- [ ] Add config struct loaded once at startup.
  - Fields: port, IPInfo token, Giphy key, MY_CIDR, timeouts.
  - Non-breaking guard: same env var names/defaults.

- [ ] Optional: test async logging with Go 1.25+ `testing/synctest` if logging worker uses timers/goroutines.
  - Use `synctest.Test` / `synctest.Wait` for deterministic concurrency tests.
  - Non-breaking guard: tests only.

- [ ] Optional: use Go 1.26 experimental `goroutineleak` profile in non-prod diagnostics.
...
- Do not change holiday date list without tests and docs.
